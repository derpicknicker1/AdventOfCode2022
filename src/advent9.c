...
static bool getInput(char *f) {
	char map[700][700] = {}, dir, a, b;
	int val, pos[10][2] = {{350,350},{350,350},{350,350},{350,350},{350,350},
	...
	while (fscanf(file, "%c %d ", &dir, &val) != EOF) {
		while (val--) {
			switch(dir) {
				case 'L': pos[0][1]--; break;
				case 'D': pos[0][0]++; break;
				case 'U': pos[0][0]--; break;
				case 'R': pos[0][1]++; break;
			}
			for ( int i = 0; i < 9; i++ )
				check(pos[i], pos[i+1], true);
			map[pos[1][0]][pos[1][1]] |= 1;
			map[pos[9][0]][pos[9][1]] |= 2;
		}	
	}
